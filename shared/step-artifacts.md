...
| **3** | `03-data-query.md` | 位次锚点、**全量预测底表**（去年投档→报考年预测→偏差）、计划调整、缺口 | 无底表或仅核心校片段 |
...
| **7** | `07-structure.md` | 分池排序说明、**同组去重**、结构门禁 | 同组重复 / 底表↔序不一致 |
| **8** | `08-candidates.md` | 推荐卡片 + [candidate-matrix](templates/candidate-matrix.md) | — |
| **9** | `09-report-draft.md` | 过程稿：画像、推荐卡、检查记录（**勿重复贴全表**） | — |
| **交付** | `FINAL-志愿方案.md` | **用户最终方案**：[final-deliverable](templates/final-deliverable.md)；**单表 45 行×10 列**（录入 5 + 解读 5：**预测、位次偏差、依据、校专综评、前景** 缺一不可）+ 预期对照 + 填报步骤 | 45 志愿省必出；**缺列=阻断交付** |
| **9+** | `11-major-outlook.md` | 前景展开（可选）；主表「前景」列已够用则可简写 | — |
| **10** | `10-preflight-check.md` | [交付前检查记录](anti-hallucination.md) + 结论 | ❌ 阻断交付 |
| **10+** | **对话内总结果** | 总结果摘要 + **FINAL 路径** + **下一步引导**（提示「交接」） | 未输出则交付不完整 |
